...
function loadServiceAccount() {
  // 1) base64 do JSON (recomendado p/ env: evita problemas com as quebras de
  //    linha da chave RSA ao colar JSON cru em variavel de ambiente).
  const b64 = process.env.FIREBASE_SERVICE_ACCOUNT_B64;
  if (b64) {
    try { return JSON.parse(Buffer.from(b64, "base64").toString("utf8")); } catch { /* tenta proximo */ }
  }
  // 2) JSON inline OU caminho de arquivo em FIREBASE_SERVICE_ACCOUNT.
  const raw = process.env.FIREBASE_SERVICE_ACCOUNT;
  if (raw && raw.trim().startsWith("{")) {
    try { return JSON.parse(raw); } catch { return null; }
  }
  // 3) arquivo em disco (fallback legado — remova apos migrar a credencial p/ env).
